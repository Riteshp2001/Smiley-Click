...
import { IS_SALE_RUNNING, SALE_SUPERHEADER_STATUS_KEY } from "@/constants";

function useSaleSuperheader() {
  // Initialize to dismissed, since we don't want to show during SSR.
  const [superheaderStatus, setSuperheaderStatus] = React.useState("dismissed");
  ...
  React.useEffect(() => {
    ...
    switch (savedSuperheaderStatus) {
      ...
      default: {
        ...
        window.localStorage.setItem(SALE_SUPERHEADER_STATUS_KEY, "seen");
      }
    }
  }, []);
  ...
  const handleDismiss = React.useCallback(() => {
    ...
    window.localStorage.setItem(SALE_SUPERHEADER_STATUS_KEY, "dismissed");
  }, []);
